...
async function recursiveExploration(page, link, depth, parentState){
  ...
  if (link.includes('/signin') || (link == 'http://localhost:2370/ghost' && depth == 0)) {
    await performLogin(page);
    console.log("passing signin");
    link = page.url();
  }
  ...
  if(!!body) {
    ...
    console.log('Errores found so far: ' + errors.length);
    ...
    if(link.includes(baseUrl)){ //Only explore pages of the specified domain
      ...
      try {
        ...
        console.log(`Failed interactWithObjects for state ${currentState}:`, error);
        errors.push({
          'url' : page.url(),
      ...
      for(const newLink of links){
        try {
          ...
          console.log(`Failed recursiveExploration for state ${currentState}:`, error);
          errors.push({
            'url' : page.url(),
            'message' : error.message,
          });
      ...
      try {
        ...
        console.log(`Failed to take screenshot for state ${currentState}:`, error);
        errors.push({
          'url' : page.url(),
          'message' : error.message,
        });
...
async function addListeners(page){
  page.on('pageerror', (err) =>{
    ...
    try {
      page.screenshot({path:capture_path, timeout: 60000});
    } catch (error) {
      console.log(`Failed to take screenshot on pageerror listener:`, error);
      errors.push({
        'url' : page.url(),
        'message' : error.message,
      });
...
async function fillInput(elementHandle, page){
  ...
  if(type === 'text'){
    ...
    elementHandle.click();
    page.keyboard.type(faker.string.alphanumeric(10));
  }
    ...
    elementHandle.click();
    page.keyboard.type(faker.number.int().toString()) ;
  }
...
async function performLogin(page) {
  ...
  await page.fill('input[name="identification"]', config.login.username);
  ...
  await page.waitForNavigation({ waitUntil: 'networkidle' }).catch(err => {
    console.log('Login might have failed or redirected improperly', err);
  });

  console.log('Login successful, proceeding with GUI ripping.');
}